...
## Live Demo — How Everything Connects
...
│                                                                 │
│  ┌─────────────────┐  ┌──────────────────┐  ┌───────────────┐  │
│  │   Dashboard     │  │  Gantt Timeline  │  │  AI Chat      │  │
│  │   KPI Metrics   │  │  Drag + Drop     │  │  Assistant    │  │
│  └────────┬────────┘  └────────┬─────────┘  └───────┬───────┘  │
│           │                   │                     │           │
│     useTelemetry          useFlights             useChat        │
│           │                   │                     │           │
│           └───────────────────┴─────────────────────┘           │
│                               │                                 │
│                        fetch() / REST                           │
└───────────────────────────────┼─────────────────────────────────┘
                   ...
                   │  GET  /flights          │
                   │  GET  /flights/{id}     │
                   │  POST /flights/reassign │
                   │  GET  /stands           │
                   │  GET  /stands/schedule  │
                   └─────────────────────────┘
```

When you drag a flight block in the Gantt to a new stand, the UI updates instantly (optimistic), the API validates the move, and if there's a conflict (size mismatch or time overlap) the block snaps back with an error message. The chat assistant understands "show me all delayed flights" and hits the real `/flights?status=delayed` endpoint — not a hardcoded response.
...
### Layered backend — routes, services, data

Routes contain zero business logic. Services contain zero HTTP imports. The `DataStore` is injected via FastAPI's DI system.

This separation means the entire service layer is testable without spinning up an HTTP server — which is why 42 tests run in under 3 seconds. Replacing the in-memory store with a real database (SQLAlchemy, etc.) requires changing only `data/mock_data.py` and `core/dependencies.py`. No service or route code changes.
