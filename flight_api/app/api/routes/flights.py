...
@router.get(
    "",
    response_model=PaginatedResponse[FlightResponse],
...
def list_flights(
...
) -> PaginatedResponse[FlightResponse]:
    try:
        result = service.list_flights(
            terminal=terminal,
            status=status,
            from_time=from_,
            to_time=to,
    ...
    flights = [FlightResponse(**f) for f in result["data"]]
    meta = PaginationMeta(**result["pagination"])
    return PaginatedResponse(data=flights, pagination=meta)
