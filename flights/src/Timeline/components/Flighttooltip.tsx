      ...
      {f && (
        <>
          ...
          <div className="mb-4 flex items-center justify-between border-b border-white/[0.08] pb-4">
            ...
            <span className={`rounded-lg border px-2.5 py-1 text-[0.65rem] font-bold uppercase tracking-widest ${STATUS_BADGE_CLASSES[f.status]}`}>
              {f.status.replace("_", " ")}
            </span>
          </div>
          ...
          <div className="flex flex-col gap-2.5 text-[0.8rem]">
            {[
              ...
              <div key={label} className="flex justify-between items-center">
                <span className="text-gray-500 font-medium tracking-wide uppercase text-[0.65rem]">
                  {label}
                </span>
                <b className="font-sans text-gray-200">{value}</b>
              </div>
            ))}
          </div>
